...
import { inputUserValidation, loginUserValidation } from "../validation/user.validation";
...
export const registerUser = async (req: Request, res: Response, next: NextFunction): Promise<Response | undefined> => {
  try {
    ...
    next(new Error("Error od file src/controllers/user.controller.ts : registerUser - " + String((error as Error).message)));
  }
};
...
export const loginUser = async (req: Request, res: Response, next: NextFunction): Promise<Response | undefined> => {
  try {
    ...
    next(new Error("Error od file src/controllers/user.controller.ts : loginUser - " + String((error as Error).message)));
  }
};
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    next(new Error("Error od file src/controllers/user.controller.ts : refreshToken - " + String((error as Error).message)));
  }
};
